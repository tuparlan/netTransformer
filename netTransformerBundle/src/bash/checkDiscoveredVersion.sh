...
checkGraphml(){
    pwd;
    keyNum=`cat network.graphml | xpath -e 'count(/graphml//key)' 2>/dev/null`;
    ...
    graphNum=`cat network.graphml | xpath -e 'count(/graphml//graph)' 2>/dev/null`;
    if [ -z "$graphNum" ]; then
       echo "There is no graphs in graphml!";
        exit 1;
    fi

    nodeNum=`cat network.graphml | xpath -e 'count(/graphml//node)' 2>/dev/null`;
    if [ -z "$nodeNum" ]; then
       echo "There is no nodes in graphml!";
        exit 1;
    fi
    edgeNum=`cat network.graphml | xpath -e 'count(/graphml//edge)' 2>/dev/null`;
    if [ -z "$nodeNum" ]; then
